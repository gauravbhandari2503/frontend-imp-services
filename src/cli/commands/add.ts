...
export async function addCommand(
  ...
  if (!service) {
    ...
    console.log(
      chalk.yellow(
        "Run 'npx frontend-services list' to see available services.",
      ),
    );
    process.exit(1);
  }
  ...
  if (service.dependencies.length > 0) {
    ...
    console.log(chalk.cyan(service.dependencies.join(", ")));
  ...
  console.log(`\nCopying files to ${chalk.gray(installPath)}...`);
  ...
  try {
    ...
    for (const fileRelativePath of service.files) {
      ...
      const sourceFile = path.resolve(
        packageRoot,
        "src",
        fileRelativePath.replace("services/", "services/"),
      );
